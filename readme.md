...
IoT 디바이스와 솔루션 백 엔드 간에 안정적이고 안전한 통신 제공
디바이스-클라우드 통신, 클라우드-디바이스 통신 모두 지원
IoT Hub Monitoring 사용하면 디바이스 만들기, 오류 및 연결과 같은 이벤트 추적하여 솔루션 상태 유지 관리

### 2. Azure IoT Central

IoT 디바이스를 연결 및 모니터링하고 관리할 수 있느 대시보드 추가하여 IoT Hub 위에 구축
시각적 UI 사용으로 확인이 용이
모든 디바이스의 전반적인 성능을 집계하고 확인, 특정 디바이스에 유지 관리가 필요할 경우 알림 설정
디바이스에 펌웨어 업데이트 푸시
일반적인 시나리오를 위한 시작 템플릿 제공
핵심 부분은 디바이스 템플릿 사용으로 서비스 쪽 코딩 없이 디바이스를 연결할 수 있음.

### 3. Azure Sphere

디바이스의 하드웨어 및 운영 체제부터 디바이스에서 메세지 허브로 메세지를 전송하는 안전한 방법까지 모두 포함하는 엔드투엔드 IoT 솔루션 구축
인터넷에 연결된 디바이스를 위한 통신 및 보안 기능이 기본 제공

세 부분으로 제공
- 연결된 센서의 신호 및 운영 체제를 처리하는 Azure Sphere MCU(마이크로 컨트롤러 유닛)
- 보안 서비스와의 통신을 처리하고 공급업체 소프트웨어 실행할 수 있는 사용자 지정된 Linux OS
- AS3라고도 하는 Azure Sphere Security Service. 디바이스가 악의적으로 손상되지 않았는지 확인하는 역할. Azure 연결을 시도 시 인증서 기반 인증을 사용하여 디바이스별 자체 인증을 수행. 인증 성공한 경우 AS3는 디바이스가 변조되지 않았는지 확인. 보안 통신 채널을 설정한 후 AS3는 OS 또는 승인된 고객 개발 소프트웨어 업데이트를 디바이스에 푸시

디바이스의 신뢰성을 검사하고 인증한 후 디바이스는 원격 분석 및 오류 정보를 전송하여 다른 Azure IoT 서비스와 상호 작용

## 2.결정 기준 분석

### 1. 디바이스가 손상되지 않았는지 확인하는 것이 중요한가?

모든 경우 그런 것은 아님. 다른 디바이스보다 무결성을 보장하는 것이 중요한 경우. 
보안이 제품 설계에서 중요한 고려 사항인 경우 최상의 제품 옵션은 IoT 디바이스에 포괄적인 엔드투엔드 솔루션을 제공하는 Azure Sphere

Azure Sphere는 하드웨어, 운영 체제, 인증 프로세스 등의 모든 것을 제어하여 디바이스와 Azure간에 보안 통신 채널을 유지
보안 채널 설정된 후 디바이스로 부터 메시지를 안전하게 받을 수 있고, 메시지 또는 소프트웨어 업데이트를 디바이스에 원격으로 전송

### 2. 보고 및 관리를 윈한 대시보드가 필요한가?

원격 분석을 수신하고 가끔 업데이트를 푸시하는 용도로만 원격 디바이스에 연결하고, 보고 기능은 필요하지 않은 경우 Azure IoT Hub 자체만 구현
IoT Hub RESTful API를 사용하여 사용자 지정 관리 도구 및 보고서들을 계속 만들수 있음

미리 빌드된, 사용자 지정 가능한 사용자 인터페이스를 통해 디바이스를 원격으로 확인하고 제어하려는 경우 IoT Central에서 시작이 좋음
이 솔루션 사용 시 디바이스 1개 또는 전체를 한번에 제어 및 오류와 같은 특정 상황에 대한 알림 설정

IoT Central은 IoT Hub를 비롯한 다양한 Azure 제품과 통합되어 보고 및 관리 기능이 있는 대시보드를 생성

## 3. IoT Hub 사용

디바이스에 원격 제어가 필요하지 않음. 
디바이스 분석 및 사전 예방 유지 관리를 위해 원격 분석 데이터 전송
유지 관리 요청을 처리하기 위한 소프트웨어가 이미 있음. 기존 시스템에 모든 기능 통합

### 1. 어떤 서비스를 선택해야 할까요?

1. 디바이스가 손상되지 않도록 하는 것이 중요한가? 
2. 보고 및 관리용 대시보드가 필요한가?

1, 2 항목에 대하여 모두 필요 없음
Azure IoT Hub가 가장 좋은 시나리오

### 2. Azure IoT Central을 사용하지 않는 이유

회사가 GUI를 통해 IoT 디바이스를 개별적으로 관리, 집계 및 보고서를 확인, 오류 알림을 설정하는 대시보드 제공
시나리오에서 수집한 원격 분석 데이터 및 다른 분석 기능을 기존 소프트웨어 애플리케이션에 통합하려함
데이터 수집만 필요. 설정이나 소프트웨어 업데이트는 필요 없음. 
Azure IoT Central 필요 없음

### 3. Azure Sphere를 사용하지 않는 이유

보안이 중요한 시나리오를 위한 솔루션
보안이 중요하지 않은 솔루션이라 필요 없음

## 4. IoT Central 사용

- 실시간 추적 및 추적을 통한 배송 모니터링
- 실시간 주변 조건 모니터링을 통해 배송 무결성
- 배송 도난, 분실 또는 손상으로부터의 보안
- 지역 펜싱, 경로치적화, 차량 관리 및 차량 분석
- 예상 가능한 배송 출발 및 도착 예측

센서 및 차량 컴퓨터 데이터를 수집하고 배송과 차량에 대한 보고서를 표시하는 그래픽 사용자 인터페이스를 제공하는 미리 빌드된 솔루션

### 1. 어떤 서비스를 선택해야 할까요?

- 디바이스가 손상되지 않도록 하는 것이 중요한가? 중요한 고려 요소가 아님
- 보고 및 관리용 대시보드가 필요한가? 필요함

Azure IoT Central을 선택하는 것이 가장 좋음

### 2. IoT Hub를 사용하지 않는 이유는 무엇인가?

IoT Central을 사용하면 커넥티드 물류 시작 템플릿을 통해 특정 요구에 맞게 미리 구성된 IoT Hub를 사용하게 됨
그렇지 않을 경우 Azure IoT Hub를 토대로 고유한 클라우드 기반 대시보드 및 관리 시스템을 빌드하기 위해 많은 사용자 지정 개발 수행 필요

### 3. Azure Sphere를 사용하지 않은 이유는 무엇인가?

보안은 있으면 좋지만 중요한 우선순위가 아님
Azure Sphere가 하드웨어를 포함하는 엔드투엔드 솔루션을 제공하지만 이 시나리오에서는 필요하지 않음

## 5. Azure Sphere 사용

자체 체크 아웃을 위한 비접촉식 point of sale 솔루션 구현
보안이 중요
중요한 정보를 보고, 원격으로 소프트웨어를 안전하게 업데이트

### 1. 어떤 서비스를 선택해야 할까요?

- 디바이스가 손상되지 않도록 하는 것이 중요한가? 중요함
- 보고 및 관리용 대시보드가 필요한가? 필요함

Azure IoT Central과 Azure Sphere 위에 플랫폼 빌드

### 2. IoT Hub를 선택하지 않는 이유는 무엇인가요?

IoT Central을 사용한다면 실제로 백그라운드에서 Azure IoT Hub도 사용하게 됨

# 8. 요구 사항에 가장 적합한 AI 서비스 선택

## 1. 제품 옵션 확인

AI는 소프트웨어 시스템이 환경을 인지하고 목표를 성공적으로 달성 할 수 있는 가능성을 최대화하는 조치를 취할 수 있도록하는 광범위한 컴퓨팅 분류
AI의 목표는 명시적으로 프로그래밍되지 않았어도 스스로 무언가를 학습하거나 무엇인가에 맞게 조정할 수 있는 소프트웨어 시스템을 만드는 것

- 인간 사고의 신경망을 기반으로 모델링한 '딥 러닝' 시스템 채택 후 경험을 통해 발견하고 학습 및 성장
- 기존 데이터를 사용하여 모델을 학습, 테스트한 다음 새 데이터 모델을 적용하여 향후 동작, 결과, 추세를 예측하는 '기계학습'
...
예측영 플랫폼
가장 정확하게 예측하는 모델을 찾기 위해 데이터에 연결하여 모델을 학습 및 테스트할 수 있는 도구와 서비스로 구성
실험을 실행하여 모델을 테스트한 후 웹 API 엔드 포인트를 통해 모델을 배포하고 실시간으로 사용

- 데이터를 가져오는 방법, 누락 및 잘못된 데이터 처리하는 방법, 데이터를 학습 세트나 테스트 세트로 분할하고 데이터를 학습 프로세스로 전달하는 방법을 정의하는 프로세스 만듦
- 데이터 과학자에게 친숙한 도구와 프로그래밍 언어를 사용하여 예측 모델을 학습시키고 평가
- 학습 데이터와 테스트 데이터를 기반으로 알고리즘의 점수를 매기는 데 필요한 컴퓨팅 집약적 실험을 실행할 위치와 시기를 정의하는 파이프라인 생성
...
애플리케이션이 보고 듣고 말하고 이해하고 추론을 시작할 수 있는 미리 빌드된 기계 학습 모델
텍스트 분석을 통한 정서적 감정 분석, 이미지 분석을 통한 개체 또는 얼굴 인식과 같은 일반적인 문제
기계 학습 또는 데이터 과학 지식 필요 없음
API를 통해 쉽게 포함

Azure Machine Learning에서는 고유 데이터를 가져와 해당 데이터에 대해 모델을 학습시켜야 하지만 
Azure Cognitive Services는 대부분의 경우 예측을 얻는 기반이 되는 라이브 데이터를 가져올 수 있드록 미리 학습된 모델 제공

- 언어 서비스 : 미리 빌드된 스크립트를 사용하여 자연어를 처리, 감정을 평가, 사용자가 원하는 것을 인식하는 방법을 학습
- 음성 서비스 : 음섬을 테스트로 그리고 텍스트를 자연스러운 음성으로 변환. 한 언어를 다른 언어로 번역하고 화자 검증 및 인식 기능을 사용하도록 설정
- 비전 서비스 : 사진, 비디오 및 다른 시각적 콘텐츠를 분석할 때 인식 및 식별 기능 추가
- 의사 결정 서비스 : 각 사용자에게 맞춤형 추천을 추가. 사용될 때마다 자동으로 개선, 콘텐츠를 모니터링해 불쾌감을 주거나 위험한 콘텐츠를 제거하여 콘텐츠를 조정, 시계열 데이터에서 비정상 상태를 탐지

### 4. Azure Bot Service

Azure Bot Service와 Bot Framework는 마치 사람처럼 질문을 이해하고 질문에 응답하는 가상 에이전트를 만들기 위한 플랫폼
사람과 지능적으로 통신 할 수 있는 가상 에이전트를 만듬
빌드한 봇은 백그라운드에서 Azure Cognitive Services와 같은 다른 Azure Service를 사용하여 사람이 요구하는 작업을 이해

사용자는 텍스트, 대화형 카드 및 음성을 사용하여 봇과 대화

## 2. 결정 기준 분석

### 1. 자연어를 통해 사용자와 상호 작용하는 가상 에이전트를 빌드하고 있나요?

자연어를 사용하여 사람과 상호 작용하는 가상 에이전트를 만들어야 하는 경우 Azure Bot Service를 사용
여러 채널 간의 상호작용을 허용하는 기술 자료, 자연어 처리 및 폼 팩터를 통합

자연어 이해, 설정 언어로 지역화하는 번역과 같은 작업을 위해 다른 AI 서비스 사용

### 2. 이미지, 비디오 또는 오디오의 콘텐츠와 의미를 이해하거나 텍스트를 다른 언어로 번역할 수 있는 서비스가 필요 합니까?

음성 텍스트 변환 수행, 검색 상호 작용 또는 이미지의 개체 식별과 같은 범용 작업을 수행할 때 Azure Cognitive Service를 사용

### 3. 사용자 행동을 예측하거나 앱에서 사용자에게 맞춤형 추천을 제공해야 하나요?

Azure Cognitive Services Personalizer 서비스는 애플리케이션 내에서 사용자의 작업을 감시
Personalizer를 사용하여 사용자의 동작을 예측, 패턴을 파악하여 관련된 경험을 제공
Azure Machine Learning 솔루션을 만들 수 있음

### 4. 앱이 프라이빗 기록 데이터를 기반으로 향후 결과 예측하나요?

미래 결과를 예측하기 위해 데이터를 분석해야 하는 경우 Azure Machine Learning을 분석해야 함

### 5. 고유한 데이터를 사용하여 모델을 빌드하거나 위에 나열된 것과 다른 자업을 수행해야 합니까?

최대 유연성을 위해 Azure Machine Learning을 사용

## 3. 의사 결정 지원 시스템에 Machine Learning 사용

체크 아웃 시저에 쇼핑객의 카트에 있는 항목을 보완하는 추가 항목 제품을 제안함으로써 판매를 늘릴수 있다고 판단
데이터 과학 전문가는 이미 문제 영역에 대해 초기 분석을 수행함
프로토타입을 생성하는데 몇 개월 정도 걸리고 출시하는 데 1년 정도 걸릴 수 있다고 판단

### 1. 어떤 서비스를 선택해야 합니까?

- 자연어를 통해 사용자와 상호 작용하는 가상 에이전트를 빌드하고 있나요? 아니요
- 이미지, 비디오, 오디오의 콘텐츠와 의미를 이해하거나 텍스트를 다른 언어로 번역할 수 있는 서비스가 필요한가요? 아니요
- 사용자 행동을 예측하거나 사용자에게 맞춤형 추천을 제공해야 하나요? 예. 그러나 사용자 행도에 따른 추천을 생성은 요구 사항의 일부. 과거 판매 데이터, 추세 판매 데이터, 재고 등을 통합한 복잡한 모델 필요
- 앱이 프라이빗 기록 데이터를 기반으로 향후 결과를 예측합니까? 예. Azure Machine Learning이 가장 적합

## 4. 데이터 분석을 위해 Cognitive Services 사용

웹 사이트 1세대는 영어로만 제공
여러 언어를 추가하면 영어 사용자와 동일한 온라인 전자상거래 경험을 비영어 사용자에게 제공할 좋은 기회

### 1. 어떤 서비스를 선택해야 합니까?

- 자연어를 통해 사용자와 상호 작용하는 가상 에이전트를 빌드하고 있나요? 아니요
- 이미지, 비디오, 오디오의 콘텐츠와 의미를 이해하거나 텍스트를 다른 언어로 번역할 수 있는 서비스가 필요한가요? 예. 텍스트 내용을 한 언어에서 다른 언어로 번역하는 작업 
- 사용자 행동을 예측하거나 사용자에게 맞춤형 추천을 제공해야 하나요? 아니오
- 앱이 프라이빗 기록 데이터를 기반으로 향후 결과를 예측합니까? 아니오

가장 적합한 제품 옵션으로 Cognitive Services를 선택

## 5. 대화형 채팅 환경에 Bot Service 사용

고객으로부터 받는 대량의 질문을 처리할 가상 에이전트

### 1. 어떤 서비스를 선택해야 합니까?

- 자연어를 통해 사용자와 상호 작용하는 가상 에이전트를 빌드하고 있나요? 예. Azure Bot Service를 사용하여 가상 에이전트 채팅 환경 구현. 서비스 감독자는 답변을 테스트하고 조정하여 계속해서 채팅 환경을 미세 조정
- 이미지, 비디오, 오디오의 콘텐츠와 의미를 이해하거나 텍스트를 다른 언어로 번역할 수 있는 서비스가 필요한가요? 예. 이 시나리오에서는 Azure Cognitive Services를 Bot Service와 함께 사용하여 솔루션 빌드할 수 있음. QnA Maker(Cognitive Services의 일부) 또는 Power Virtual Agent와 같은 미리 빌드된 솔루션 사용. LUIS(Language Understanding)와 같은 일부 Azure Cognitive Services를 구현 번역기.
- 사용자 행동을 예측하거나 사용자에게 맞춤형 추천을 제공해야 하나요? 아니오
- 앱이 프라이빗 기록 데이터를 기반으로 향후 결과를 예측합니까? 아니오

# 9. Azure 서버리스 기술 검토 및 비즈니스 시나리오에 적합한 서비스 선택

## 1. 소개

서버리스 컴퓨팅은 사용자를 위해 설정 및 관리되는 실행 환경을 설명하는데 사용되는 용어
코드를 작성하거나 시각적 개체 편집기에서 구성 요소를 연결 및 구성하여 자신이 원하는 작업을 지정한 다음, 타이머 또는 HTTP 요청과 같은 기능을 트리거하는 동작을 지정
중단에 대한 걱정할 필요 없으며 수요에 맞게 코드를 즉시 확장
코드의 실제 사용량에 대해서만 비용을 지불

## 2. 제품 옵션 확인

코드를 실행하는 클라우드에 호스팅된 실행 환경
기본 호스팅 환경을 추상화
서버리스 컴퓨팅이라는 용어는 부적절, 코드 또는 원하는 기능을 실행하는 서버가 있음
핵심 개념은 사용자가 서버를 설정하거나 유지 관리할 책임이 없다는 점
일반적인 백 엔드 시나리오를 처리하는 데 사용
Azure Functions 및 Azure Logic Apps 등

### 1. Azure Functions

이벤트에 대한 응답으로 실행되는 클라우드에서 널리 사용되는 프로그래밍 언어를 사용하여 단일 메서드 또는 함수를 호스트
원자적 특성 때문에 Azure Functions는 애플리케이션의 설계에서 다양한 용도로 사용
자동으로 크기 조정되며 함수가 트리거될 때만 요금이 누적
상태 비저장 환경. 이벤트 응답할 때마다 다시 시작되는 것처럼 작동
Durable Functions라는 확장을 사용하여 오케스트레이션 작업을 수행할 수 있으므로 상태 유지하면서 Functions를 함께 연결할 수 있음
서비스를 실행에만 관심이 있고 기본 플랫폼이나 인프라에는 관심이 없는 경우에 적합
이벤트에 대한 응답으로 작업을 수행해야 하는 경우에 일반적으로 사용

### 2. Azure Logic Apps

클라우드로 호스트되는 로우 코드/코드 없는 개발 플랫폼
앱, 데이터, 시스템 및 서비스를 통합해야 하는 경우 작업, 비즈니스 프로세스 및 워크플로를 자동화하고 오케스트레이션하는 데 유용
앱 통합, 데이터 통합, 시스템 통합, EAI, B2B 통합을 다룸

웹 기반 디자이너에서 설계되며 코드 작성 없이 트리거된 로직을 실행
커넥터를 사용하여 트리거를 작업에 연결하여 앱을 빌드
트리거는 앱을 실행한 다음 새 메시지를 큐로 보내거나 HTTP 요청을 발생시키는 이벤트

### 3. 이러한 서비스의 차이점은 무엇인가요?

Azure Logic Apps에서 Azure Functions를 호출할 수 있고 그 반대의 경우도 마찬가지
두 서비스의 주요 차이점은 서비스의 인텐트
Azure Functions는 서버리스 컴퓨팅 서비스, Azure Logic Apps는 서버리스 오케스트레이션 서비스
Azure Functions를 사용 다양한 연결이 포함된 장기 실행 비즈니스 프로세스를 오케스트레이션할 수는 있지만 기본 사용 사례는 아님

Azure Functions는 실행 횟수 및 각 실행의 실행 시간을 기반으로 가격 책정
Azure Logic Apps는 실행 횟수 및 사용되는 커넥터 유형을 기반으로 가격 책정

## 3. 결정 기준 분석

### 1. 잘 알려진 API에서 오케스트레이션을 수행해야 하나요?

Azure Logic Apps는 웹 기반 시각적 개체 구성자에서 가격 책정 모델에 이르기까지 오케스트레이션을 염두에 두고 설계
API를 통해 다양한 이질적인 서비스를 연결하여 워크플로의 여러 단계를 통해 데이터를 전달하고 처리

Azure Functions를 사용하여 동일한 워크플로를 만들 수 있지만 호출할 API와 호출 방법을 조사하는 데 상당한 시간이 걸릴 수 있음.

### 2. 사용자 지정 알고리즘을 실행하거나 특수한 데이터 구문 분석 및 데이터 조회를 수행해야 하나요?

Azure Functions를 사용하면 프로그래밍 언어의 전체 표현력을 간결한 형태로 사용
알고리즘 또는 데이터 조회 및 구문 분석 작업을 간결하게 빌드
사용자는 코드를 유지 관리하고 탄력적으로 예외 처리

Azure Logic Apps는 로직을 수행할 수 있지만 복잡한 알고리즘이 필요한 로직이 많은 오케스트레이션이 있는 경우 알고리즘을 구현하려면 일이 많고 시각적으로 압도

### 3. 기존의 자동화된 작업이 명령형 프로그래밍 언어로 작성되어 있습니까?

다른 인기 있는 프로그래밍 언어로 표현된 오케스트레이션 또는 비즈니스 로직이 있다며 Azure Functions 함수 앱의 본문으로 이식하는 것이 쉬움

### 4. 코드 작성(명령형)보다 시각적 개체(선언적) 워크플로를 선호하나요?

궁극적으로 선언적 환경에서 작업할지 또는 명령적 환경에서 작업할지에 따라 선택 결정

## 4. Azure Functions 사용

각 제품에 대한 데이터는 JSON 메시지로 패키지되고 이벤트 허브에 전송
다양한 시스템에 알림을 보낼 수 있도록 JSON 메시지를 구독자에게 배포

매일 밤 오전 2:00 제품 가용성 업데이트
- 메시지 검색
- JSON 구문 분석
- 여러 데이터베이스에서 조회를 수행하여 추가 제품 정보 찾기
- 잠재적 수량이 특정 수준 이하로 떨어지면 재주문할 수 있도록 구매 부서 알림

재고 부족 현상이 하루에도 여러 번 발행
알고리즘 실행을 제품 구매할 때마다 재고 업데이트 프로그램 실행

### 1. 어떤 서비스를 선택해야 할까요?

이미 작성된 코드가 있으므로 Azure Functions로 이식

### 2. Azure Logic Apps를 선택하지 않는 이유는 무엇인가요?

서비스를 구축하는데 이미 시간을 투자 했음

## 5. Azure Logic Apps 사용

무작위 고객 만족도 설문 조사 초대장 발송
고객 만족도 결과 집계 후 평균값 차트 작성

고객 만족도 점수가 부정적이면 고객 유지 워크플로를 트리거
댓글 감정 분석이 생성, 사과와 쿠폰 코드가 포함된 이메일 고객에게 전송 후 이메일 예약을 위한 Dynamics 365 고객 서비스로 메시지 라우팅
개발자 리소스 없음

### 1. 어떤 서비스를 선택해야 할까요?

Azure Logic Apps가 최상의 솔루션
Azure Cognitive Services 커넥터로 감정 분석을 A수행 Office 365 Outlook 커넥터를 사용하여 이메일을 발송
Dynamics 365 고객 서비스 커넥터를 사용하여 새 레코드 및 후속 이메일 생성

### 2. Azure Functions를 선택하지 않느 이유는 무엇인가요?

Azure Functions를 사용하여 전체 솔루션을 빌드하는 것이 가능하지만 소프트웨어 개발자가 없으면 어려움
워크플로의 단계에 맞는 커넥터가 이미 있음
모두 활용하는 솔루션을 빌드하려면 개발자가 연구, 개발, 테스트에 많은 시간 할애 필요

# 10. 조직에서 더 나은 솔루션을 빌드하는 데 도움이 되는 최상의 도구 선택

## 1. 소개

## 2. 제품 옵션 이해

DevOps는 여러 기술 팀이 공동의 목표를 달성하기 위해 협력하는 것을 지원하기 위한 철학과 사계가 결합된 개념

### 1. 제품 옵션

도구 세트, 서비스 세트, 프로그래미틱 API 세트

### 2. Azure DevOps Services

소프트웨어 개발 수명 주기의 모든 단계에서 문제를 해결하는 서비스 모음

- Azure Repos: 코드 게시할 수 있는 중앙형 코드 리포지토리
- Azure Boards: Kanban보드, 아이디어 보고 및 추적, 작업을 포함하는 프로젝트 관리 제품군
- Azure Pipeline: CI/CD 파이프라인 자동화 도구
...
### 3. GitHub 및 GitHub Actions

오픈 소스 소프트웨어를 위한 세계에서 가장 인기 있는 코드 리포지토리
Git은 분산형 소스 코드 관리 도우인 반면, GitHub는 기본적으로 원격으로 사용되는 호스티드 버전의 Git

- 기본 코드 베이스로 병합하기 전에 소스 코드의 웹 보기에서 개발자가 주석과 질문을 추가하여 코드 검토를 수행할 수 있는 도구를 포함하는 공유 소스 코드 리포지토리
- Kanban 보드를 비롯한 프로젝트 관리 용이
- 문제 보고, 토론 및 추적을 지원
- CI/CD 파이프라인 자동화 도구 제공
- 협업 설명서를 위한 Wiki 포함
- 클라우드 또는 온-프레미스에서 실행

GitHub Actions에서는 많은 수명 주기 이벤트의 트리거를 사용하여 워크플로를 자동화할 수 있음
CI/CD '툴체인' 자동화는 시스템 개발 수명 주기 전반에 걸쳐 소프트웨어 애플리케이션의 제공, 개발 및 관리를 지원하는 소프트웨어 도구의 조합
툴체인의 한 도구가 출력되면 툴체인의 다음 도구가 입력
일반적인 도구 기능은 자동화된 종속성 업데이트 수행부터 소프트웨어 빌드 및 구성, 다양한 위치로 빌드 아티팩트 전달, 테스트 등

### 4. Azure DevTest Labs

소프트웨어 프로젝트의 빌드가 포함된 VM을 빌드, 설정 및 삭제하는 프로세스를 관리하기 위한 자동화된 방법을 제공

## 3. 결정 기준 분석

### 1. 테스트 랩 만들기를 자동화하고 관리해야 합니까?

테스트 랩 환경 만들기 및 관리를 자동화하는 것이 목표인 경우 Azure DevTest Labs를 선택
Azure Pipelines 또는 GitHub Actions를 사용하여 새 랩의 프로비저닝을 툴체인의 일부로 자동화할 수 있음

### 2. 오픈 소스 소프트웨어를 빌드하고 있나요?

오픈 소스 소프트웨어를 빌드하는 GitHub가 좋은 선택

### 3. 소스 코드 관리 및 DevOps 도구와 관련하여 권한에 어떤 수준의 세분성이 필요합니까?

GitHub에서는 모든 기능에 대한 읽기 쓰기 권한 모델이 간단
Azure DevOps에는 훨씬 더 세분화된 권한 세트가 있으며, 구체적으로 지정 가능

### 4. 소스 코드 관리 및 DevOps 도구와 관련하여 프로젝트 관리 및 보고가 얼마나 정교해야 합니까?

프로젝트 관리 및 보고에서는 Azure DevOps

### 5. 소스 코드 관리 및 DevOps 도구와 관련하여 타사 도구와 얼마나 긴밀하게 통합해야 합니까?

DevOps 도구를 만드는 대부분의 공급업체는 Azure Pipeline 및 GitHub Actions에서 둘 다 사용할 수 있는 후크 또는 API를 만들수 있음
